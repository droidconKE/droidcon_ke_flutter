...
class FavoritesProvider with ChangeNotifier {
  ...
  FavoritesProvider() {
    userAuthSub = FirebaseAuth.instance.onAuthStateChanged.listen((newUser) async {
      ...
      if(newUser == null){
        ...
        Firestore.instance.collection('starred_sessions')
            .where('user_id', isEqualTo: newUser.uid)
           .snapshots().listen(((data){
             sessions = data.documents.map((doc) => StarredSession.fromFirestoreDocument(doc)).toList();
             notifyListeners();
        }));
      }
    }, onError: (e) {
